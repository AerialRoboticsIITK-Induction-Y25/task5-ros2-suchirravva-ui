...
public:
    AutonomousDrone(const std::string& v_name, float initial_battery, float max_alt, float spd, const std::string& m_name, std::tuple<float, float, float> home);
    ...
    std::string get_ai_mode() const { return ai_mode; }
};
